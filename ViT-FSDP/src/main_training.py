        ...
        for param_name, param in submodule.named_parameters(recurse=False):
            if not _is_fsdp_flattened(param) and param.is_meta:
                materialized_param = nn.Parameter(
                    torch.empty_like(param, device=torch.device("cuda:0"))
                )
...
def fsdp_main():
    ...
    # Overwrite config file from args
    if (args.model=='vitsmart'):
        cfg.model_name = args.model_size        
        cfg.batch_size_training = args.bs
        cfg.image_size = args.image_size
        cfg.use_tp = args.use_tp
        cfg.use_ddp = args.use_ddp
        if (args.strategy=="NO_SHARD"):
            cfg.sharding_strategy = ShardingStrategy.NO_SHARD
        elif (args.strategy=="FULL_SHARD"):
            cfg.sharding_strategy = ShardingStrategy.FULL_SHARD
        elif (args.strategy=="SHARD_GRAD_OP"):
            cfg.sharding_strategy = ShardingStrategy.SHARD_GRAD_OP
        elif (args.strategy=="HYBRID_SHARD"):
            cfg.sharding_strategy = ShardingStrategy.HYBRID_SHARD
        else:
            print("Strategy not supported yet")
            exit(-1)
    ...
    if (args.strategy=="HYBRID_SHARD" and args.scaling_group_size>0):        
        from torch.distributed._tensor import DeviceMesh

        scaling_group_size = args.scaling_group_size
        mesh_list = []
        dmesh = torch.arange(0, world_size).view(-1, scaling_group_size)
        for i, sg in enumerate(dmesh):
            mesh_list.append(sg.tolist())

        print("mesh", rank, mesh_list, flush=True)
        ...
        process_group_fsdp = (shard_group, replicate_group)
    ...
    #
    print("Data sample size:", len(data_loader))
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="base",
        metavar="string",
        choices=["base", "huge", "giant", "enormous", "5B", "15B"],
    )    
    parser.add_argument(
        "--strategy",
        default="NO_SHARD",
        metavar="string",
        choices=["NO_SHARD", "FULL_SHARD", "SHARD_GRAD_OP", "HYBRID_SHARD"],
    )
    parser.add_argument(
        "--scaling_group_size",
        default=0,
