...
class train_config(base_config):
    ...
    use_timm = False
    use_parallel_attention: bool = False #True

    # only relevant if use_parallel_attention True
    use_multi_query_attention: bool = True #False
    ...
    batch_size_training: int = 64 #40 #64
    # validation
...
def build_model(
    ...
    print(f"{local_cfg.num_categories=}")
    NUM_CLASSES = local_cfg.num_categories
...
class GeneratedDataset(Dataset):
    def __init__(self, **kwargs) -> None:
        ...
        self._len = kwargs.get("len", 1282168)
...
def train(
    ...
    for batch_index, (batch) in enumerate(data_loader, start=1):
        ...
        # End of batch
        batch_index_count+=1
        if(batch_index_count==step2go): break
    ...
    if(rank==0):
        print("EASY_LOG",
              len(data_loader),
              (t1_epoch-t0_epoch),
              #(len(data_loader)*data_loader.batch_size*world_size) /(t1_epoch-t0_epoch),
              (step2go*data_loader.batch_size*world_size) /(t1_epoch-t0_epoch),
              flush=True)
